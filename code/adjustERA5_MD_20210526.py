...
for year in range(1979, 2020+1):
    ...
    msnswrfcs = get_shifted_data('msnswrfcs', year)  # this should be msnswrf
    msnlwrfcs = get_shifted_data('msnlwrfcs', year)  # this should be msnlwrf
    ...
    with nc.Dataset(outfile,'r+') as ncf:
        ...
        ncf['msr'][:,:,:] = msr
        ncf['mtpr'][:, :, :] = mtpr
        ncf['msnswrfcs'][:, :, :] = msnswrfcs
        ncf['msnlwrfcs'][:, :, :] = msnlwrfcs
